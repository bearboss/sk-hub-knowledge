...
```
document.write("<script src=\"/assets/js/hook.js\"></script>");

window.onload = function () {
    if (ah) {
        var userName = JSON.parse(window.localStorage.getItem("easyweb-iframe")).loginUser.nickName;
        ah.proxy({
            //请求发起前进入
            onRequest: (config, handler) => {
                var urlName = config.url
                window._paq.push(['setCustomUrl', urlName])
